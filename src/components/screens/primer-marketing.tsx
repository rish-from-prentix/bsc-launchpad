...
export function PrimerMarketing({ onBack, onQuiz }: { onBack: () => void; onQuiz: () => void }) {
  return (
    <div className="mx-auto max-w-2xl px-5 sm:px-8 py-10 sm:py-14" style={{ animation: "fadeSlide 250ms ease-out" }}>
      ...
      <h1 className="text-[28px] sm:text-[32px] font-bold tracking-tight leading-tight">Marketing Elasticity</h1>
      ...
      <div className="mt-10">
        ...
        <FormulaCard>Marketing Elasticity = (% change in sales) / (% change in marketing spend)</FormulaCard>
        <p className="text-[15px] text-foreground/90 leading-[1.7]">
          When a report says marketing elasticity
          <span className="font-mono text-primary">(m) = 1.6</span>, here’s how you read it: <br />
          If you increase the marketing spend by 1%, your total sales are likely to increase by 1.6%.
        </p>
      </div>
          ...
          <table className="w-full text-sm font-mono">
            <thead>
              <tr className="text-primary text-[11px] uppercase tracking-[0.14em]">
                <th className="text-left font-medium px-4 py-3">Marketing Spend</th>
                <th className="text-left font-medium px-4 py-3">Change</th>
                <th className="text-left font-medium px-4 py-3">Expected Sales</th>
              </tr>
            </thead>
            <tbody>
              {[
